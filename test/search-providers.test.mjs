...
test("Brave search applies domain filters in the query and returned results", async () => {
	...
	const child = runChild(`
		...
		globalThis.fetch = async (url, init) => {
			...
			return new Response(JSON.stringify({
				web: { results: [
					{ title: "GitHub", url: "https://github.com/mahdyarief/pi-web-access", description: "repo" },
					{ title: "Gist", url: "https://gist.github.com/mahdyarief/abc", description: "gist" },
	...
	assert.deepEqual(output.results.map((result) => result.url), ["https://github.com/mahdyarief/pi-web-access"]);
});
